...
export function featureCtx({ id, manifest, snapshot, previous = null, deadline, onProgress = () => {} }) {
  ...
  return {
    ...
    timeouts: TIMEOUTS,
    // The slow lane for the rare call that legitimately needs more than the
    // default (a live fetch of every domain, a 90-day curve): the slow timeout,
    // never past the step deadline minus `marginMs`, never under 1 s.
    slowTimeout: (marginMs = 2000) => Math.max(1000, Math.min(TIMEOUTS.slow, deadline - Date.now() - marginMs)),
