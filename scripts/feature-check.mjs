  ...
  if (m.server) {
    ...
    if (typeof mod.build === 'function') {
      ...
      const sctx = {
        ...
        snapshot: demoSnap, previous: null, deadline: Date.now() + 60000, timeLeft: () => 60000, timeouts: TIMEOUTS, slowTimeout: (m = 2000) => Math.max(1000, Math.min(TIMEOUTS.slow, 60000 - m)),
