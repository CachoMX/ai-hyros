...
**id** `funnel` · **mode** demo · **version** 1.1.0
...
- Reads `snapshot.ranges['30d']` — totals (`clicks`, `carts`, `leads`,
  `uniqueCustomers`, `sales`, `revenue`, `refund`, `refundCount`) and the
  campaign level rows.
- **Order value (1.1.0)**: `orders = sales`, `gross = revenue`,
  `net = gross − refund` (floored at 0). Net AOV = `net ÷ orders`; the
  value of an add to cart = `net ÷ carts`; every stage carries
  `netPer = net ÷ stage.value`. Refunds are subtracted from the gross
  BEFORE any per-unit figure is derived, so a refund lowers the AOV and
  the add-to-cart value together. Refunded orders still count as orders
  (the money came off, the order happened).
...
```json
{ "stages": [{ "label": "", "sub": "", "value": 0, "unit": "add to cart", "netPer": 0.0 }],
  "value": { "orders": 0, "gross": 0, "refunds": 0, "refundCount": 0, "net": 0,
             "aovGross": 0.0, "aov": 0.0, "perClick": 0.0, "perCart": 0.0, "perLead": 0.0, "perCustomer": 0.0 },
...
- A block without `value` (older packs) renders the funnel without the
  value column and ledger — the view degrades, never throws.
