...
Deploy it to your own Vercel account; the first load walks you through a
password and your HYROS API key, and a **Demo account** is always there to
explore before (or without) connecting anything.

<!-- Deploy button: point it at the HYROS template repository before the
     public release. The clone URL must carry the Upstash store parameter
     (`stores=[{"type":"integration","integrationSlug":"upstash","productSlug":"upstash-redis"}]`). -->
...
This is template version **0.2.0** (`package.json`; also returned by
...
1. **Get the code into Vercel.** Beta flow: upload the HYROS template
   repository to your own GitHub (or fork it — see *Getting updates*),
   then in Vercel choose **Add New → Project → Import** that repository.
   Framework preset *Other*, no build step, output directory `public`.
   When the HYROS template repository ships a Deploy button, it does the
   same import plus the storage step below in one click.
2. **Add storage — Upstash for Redis.** In the Vercel project open
   **Storage → Create Database → Upstash for Redis** (from the Vercel
   marketplace), connect it to the project for all environments, then
   **Redeploy**. This injects `KV_REST_API_URL` / `KV_REST_API_TOKEN`
   (or `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`; either
   naming works). That store is the ONLY thing the app needs from Vercel:
   it holds snapshots, settings and your encrypted API keys.
...
3. **Open the URL.** The first load is one screen: **paste your HYROS API
   key** and **choose the dashboard password**. Tick *agency key* to add
   every client account you have access to. The first snapshot builds right
   away. Nothing is set in Vercel — the password lives in your database.
4. **Optional hardening.** Two secrets were generated for you and stored in
...
- **Fork on GitHub:** use the **Sync fork** button on your repository page
  to pull the latest template commits, then Vercel redeploys automatically.
- **Uploaded copy:** add the template as a remote once and merge:
  ```bash
  git remote add upstream <HYROS template repository URL>
  git fetch upstream
...
- **Report a problem** in Setup & security links to the template's issue
  tracker.
- `/api/health?key=<password>` proves the MCP leg without building a
  snapshot: tool count, `missingTools`, account email and timezone.
