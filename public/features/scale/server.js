...
const curveTimeout = (ctx) => (typeof ctx.slowTimeout === 'function' ? ctx.slowTimeout(2000) : Math.max(1000, Math.min(PER_CALL_TIMEOUT_MS, ctx.timeLeft() - 2000)));
...
export async function build(ctx) {
  ...
  for (const t of targetsFor(snapshot)) {
    ...
    try {
      ...
      const raw = await callTool('hyros_get_marginal_cac_curve', { request }, { timeoutMs: curveTimeout(ctx) });
